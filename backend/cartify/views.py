...
def add_item(request):
    try:
        ...
        return Response(
            {"data": serializer.data, "message": "Cart Item created"}, status=201
        )
...
def get_cart_stat(request):
    ...
    cart, created = Cart.objects.get_or_create(
        cart_code=cart_code, defaults={"paid": False}
    )
...
def delete_cartitem(request):
    ...
    if not cartitem_id:
        return Response(
            {"error": "Missing item_id"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    try:
        ...
        cartitem.delete()
        return Response(
            {"message": "CartItem deleted"}, status=status.HTTP_204_NO_CONTENT
        )
...
def create_product(request):
    ...
    if serializer.is_valid():
        product = serializer.save(
            seller=request.user, slug=slugify(serializer.validated_data["name"])
        )
        return Response(
            {"message": "Product created", "id": product.id},
...
def seller_dashboard_data(request):
    ...
    for product in products:
        ...
        chart_data.append({"name": product.name, "quantity": qty})
        total_sales += qty
        earnings += revenue

    return Response(
        {"total_sales": total_sales, "earnings": earnings, "chart_data": chart_data}
    )
...
class HelpCustomView(APIView):
    def post(self, request):
        ...
        answer = answer_map.get(
            qid, "Sorry, I don't have an answer for that question yet."
        )
